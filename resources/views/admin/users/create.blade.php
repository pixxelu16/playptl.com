...
@section('content')
    <section class="admin-card">
        <div class="admin-page-header" style="margin-bottom: 24px;">
            <div>
                <h1 class="admin-card-title">Add New User</h1>
                <p class="admin-card-text">Create a new user account, select their account type, and assign custom roles/permissions.</p>
            </div>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="admin-form">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div class="admin-form-group">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="admin-form-input" style="padding: 10px; border: 1px solid #ddd; border-radius: 6px; width: 100%;" required placeholder="Full Name">
                </div>
                ...
                <div class="admin-form-group">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" class="admin-form-input" style="padding: 10px; border: 1px solid #ddd; border-radius: 6px; width: 100%;" required placeholder="user@example.com">
                </div>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div class="admin-form-group">
                    <label for="password" class="admin-form-label" style="font-weight: 600; display: block; margin-bottom: 6px;">Password <span style="color: red;">*</span></label>
                    <input type="password" name="password" id="password" class="admin-form-input" style="padding: 10px; border: 1px solid #ddd; border-radius: 6px; width: 100%;" required placeholder="Minimum 8 characters">
                </div>
                ...
                <div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 8px; max-height: 250px; overflow-y: auto; padding: 12px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px;">
                        @foreach($permissions as $perm)
                            <label style="display: inline-flex; align-items: center; gap: 8px; font-size: 14px; color: #334155; cursor: pointer;">
                                <input type="checkbox" name="spatie_permissions[]" value="{{ $perm->name }}" {{ is_array(old('spatie_permissions')) && in_array($perm->name, old('spatie_permissions')) ? 'checked' : '' }} style="border-radius: 4px; border: 1px solid #cbd5e1; width: 16px; height: 16px;">
                                <span>{{ $perm->name }}</span>
                            </label>
                        @endforeach
                    </div>
            ...
            <div style="margin-top: 30px; display: flex; gap: 12px;">
                <button type="submit" class="admin-button">
                    <i class="fa-solid fa-check" aria-hidden="true" style="margin-right: 6px;"></i>
