        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="admin-form">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                ...
                <div class="admin-form-group">
                    ...
                    @php
                        $currentRoleStr = is_object($user->role) && property_exists($user->role, 'value') ? $user->role->value : (string) $user->role;
                    @endphp
                    <select name="role" id="role" class="admin-form-input" style="padding: 10px; border: 1px solid #ddd; border-radius: 6px; width: 100%; height: 43px;" required>
                        @foreach($allRoles as $rName)
                            <option value="{{ $rName }}" {{ old('role', ucfirst($currentRoleStr)) === $rName || strtolower(old('role', $currentRoleStr)) === strtolower($rName) ? 'selected' : '' }}>{{ $rName }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                    ...
                    <div class="admin-form-group">
                        <select name="spatie_role" id="spatie_role" class="admin-form-input" style="padding: 10px; border: 1px solid #ddd; border-radius: 6px; width: 100%; height: 43px;">
                            ...
                            @foreach($adminRoles as $adminRole)
                                <option value="{{ $adminRole->name }}" {{ old('spatie_role', $userSpatieRole) === $adminRole->name ? 'selected' : '' }}>{{ $adminRole->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 8px; max-height: 250px; overflow-y: auto; padding: 12px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px;">
                        @foreach($permissions as $perm)
                            <label style="display: inline-flex; align-items: center; gap: 8px; font-size: 14px; color: #334155; cursor: pointer;">
                                <input type="checkbox" name="spatie_permissions[]" value="{{ $perm->name }}" {{ in_array($perm->name, old('spatie_permissions', $userPermissions), true) ? 'checked' : '' }} style="border-radius: 4px; border: 1px solid #cbd5e1; width: 16px; height: 16px;">
                                <span>{{ $perm->name }}</span>
                            </label>
                        @endforeach
                    </div>
