...
<div class="space-y-6">
    ...
    {{-- Back --}}
    <a href="{{ route('student.bookings') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-[#5DA44E] hover:underline">
    ...
    <div class="overflow-hidden rounded-[12px] bg-white shadow-[0_1px_8px_rgba(0,0,0,0.06)] ring-1 ring-[#E0E0E0]">
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div>
                <h3 class="text-xl font-bold text-gray-900">Booking with {{ $booking->provider->name }}</h3>
                <p class="text-sm text-gray-500 capitalize">{{ $booking->provider_type }} Session</p>
            </div>
            <span class="px-4 py-1.5 rounded-full text-sm font-bold border {{ $sc }}">
                {{ $booking->statusLabel() }}
            </span>
        </div>
                ...
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $booking->from_date->format('M d, Y') }} → {{ $booking->to_date->format('M d, Y') }}</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <i class="fa-solid fa-clock text-[#5DA44E] mt-0.5 w-4"></i>
                        <div>
                            <p class="text-xs text-gray-400">Hours / Day</p>
                            <p class="text-sm font-semibold text-gray-800">{{ $booking->hours_per_day }} Hours</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <i class="fa-solid fa-clock text-[#5DA44E] mt-0.5 w-4"></i>
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $booking->total_days }} days × {{ $booking->hours_per_day }} hrs/day = {{ $booking->total_hours }} total hrs</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs text-gray-400">Preferred Time</p>
                            <p class="text-sm font-semibold text-gray-800">{{ Carbon\Carbon::parse($booking->booking_time)->format('g:i A') }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs text-gray-400">Your Location</p>
                            <p class="text-sm font-semibold text-gray-800">{{ $booking->student_location }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs text-gray-400">Your Phone</p>
                            <p class="text-sm font-semibold text-gray-800">{{ $booking->student_phone }}</p>
                        </div>
                    </div>
                    @endif
                    @if($booking->message)
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs text-gray-400">Your Message</p>
                            <p class="text-sm text-gray-700 italic">"{{ $booking->message }}"</p>
                        </div>
                    </div>
                    @endif
                </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-gray-800">@if($booking->hourly_rate > 0) {{ $currencySymbol }}{{ number_format($booking->hourly_rate, 2) }}/hr @else Free @endif</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Total Hours</span>
                        <span class="font-semibold text-gray-800">{{ $booking->total_hours }} hrs</span>
                    </div>
                    <div class="border-t border-gray-100 pt-2 mt-2 flex justify-between items-center">
                        <span class="font-bold text-gray-700">Total Charged</span>
                        <span class="text-xl font-extrabold text-[#5DA44E]">
                    ...
                    <div class="flex justify-between text-red-600">
                        <span class="font-semibold">Refund Issued</span>
                        <span class="font-bold">{{ $currencySymbol }}{{ number_format($booking->total_amount, 2) }}</span>
                    </div>
                    @endif
                </div>

                {{-- Booked On --}}
                <p class="text-xs text-gray-400 mt-4">Booked on {{ $booking->created_at->format('M d, Y \a\t g:i A') }}</p>
